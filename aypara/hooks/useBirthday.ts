...
const useBirthday = () => {
  ...
  const onAddBirthday = async (name: string, date: string, phone?: string) => {
    const id = generateId();
    ...
    if (settings.notificationsEnabled && settings.birthdayNotifications) {
      ...
      if (notifDate < now) {
        notifDate.setFullYear(notifDate.getFullYear());
      }

      const formattedDate = formatDateToCustomString(notifDate, lang);
      const bodyMessage = t("birthday_notification_today_body").replace(
        "{{name}}",
        name,
      );
  ...
  const onRescheduleBirthdayNotification = async (
    birthdayId: string,
    newDate: Date,
  ) => {
    ...
    const rescheduleMessage = t("birthday_notification_today_body").replace(
      "{{name}}",
      displayName,
    );
