...
const BirthdayHistoryPage = () => {
  ...
  const historyItems = useMemo(() => {
    ...
    return results.sort(
      (a, b) => b.occurrenceDate.getTime() - a.occurrenceDate.getTime(),
    );
  }, [birthdays]);
  ...
  const filteredItems = useMemo(() => {
    ...
    return historyItems.filter((item) => {
      const wasGreeted =
        (item.greetingYear === item.occurrenceYear && item.greetingSent) ||
        (item.greetedHistory &&
          item.greetedHistory.includes(item.occurrenceYear));
  ...
  const [confirmTarget, setConfirmTarget] = React.useState<HistoryItem | null>(
    null,
  );
  ...
  const renderItem = ({ item }: { item: HistoryItem }) => {
    const wasGreeted =
      ...
      (item.greetedHistory &&
        item.greetedHistory.includes(item.occurrenceYear));
    const isUnread =
      !item.readHistory || !item.readHistory.includes(item.occurrenceYear);
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => {
          if (isUnread) {
            dispatch(
              markHistoryRead({ id: item.id, year: item.occurrenceYear }),
            );
          }
        }}
        style={[
        ...
        <View style={styles.cardContent}>
          ...
          <View style={styles.infoRow}>
            <StyledText
              style={[
                styles.dateText,
                { color: colors.SECTION_TEXT, fontSize: 11 },
              ]}
            >
          ...
          >
            <Ionicons
              name={
                wasGreeted ? "checkmark-done-circle" : "close-circle-outline"
              }
              size={12}
            ...
            >
              {wasGreeted
                ? t("stats_birthday_greeted")
                : t("stats_birthday_missed")}
            </StyledText>
          </View>
        </View>
  ...
  return (
    <View
      style={[styles.container, { backgroundColor: colors.PRIMARY_BACKGROUND }]}
    >
      <StyledHeader
        title={t("birthday_history")}
        rightSection={
          historyItems.some(
            (item) => !item.readHistory?.includes(item.occurrenceYear),
          ) && (
      ...
      <FlatList
        ...
        keyExtractor={(item, index) =>
          `${item.id}-${item.occurrenceYear}-${index}`
        }
