...
import SortControls, {
  SortBy,
  SortOrder,
} from "../../todo/TodoList/SortControls";
...
import { BIRTHDAY_LIGHT, getStyles } from "./styles";
...
const BirthdayList: React.FC<BirthdayListProps> = ({
  ...
  const [rescheduleTarget, setRescheduleTarget] = useState<Birthday | null>(
    null,
  );
  ...
  const otherBirthdays = useMemo(() => {
    ...
    const filtered = birthdays.filter(
      (b) => !todayIds.has(b.id) && !upcomingIds.has(b.id),
    );
  ...
  useEffect(() => {
    if (todayBirthdays.length === 0) {
      ...
      Animated.timing(todayAnimation, {
        toValue: 0,
        duration: 300,
        useNativeDriver: true,
      }).start();
    }
  }, [todayBirthdays.length, todayAnimation]);
  ...
  }, [upcomingBirthdays.length, upcomingAnimation]);

  useEffect(() => {
    if (otherBirthdays.length === 0) {
      setAllExpanded(false);
      Animated.timing(allAnimation, {
        toValue: 0,
        duration: 300,
        useNativeDriver: true,
      }).start();
    }
  }, [otherBirthdays.length, allAnimation]);
  ...
  const getDaysColor = (days: number) => {
    ...
    if (days === 1) return colors.PLACEHOLDER_SECOND;
  ...
  const BirthdayGridItem = ({ item }: { item: Birthday }) => {
    ...
    const notification = useAppSelector((state) =>
      item.notificationId
        ? selectNotificationById(state, item.notificationId)
        : undefined,
    );
    const reminderStatus = notification?.status;
    const isReminderCancelled =
    ...
    return (
      ...
      >
        <View style={styles.gridCardHeader}>
          ...
          >
            {isToday ? (
              ...
              <StyledText
                style={[styles.gridAvatarText, { color: colors.PRIMARY_TEXT }]}
              >
          ...
          <View
            style={[
              styles.gridDaysBadge,
              { backgroundColor: getDaysColor(daysUntil) + "20" },
            ]}
          >
            <StyledText
              style={[styles.gridDaysText, { color: getDaysColor(daysUntil) }]}
            >
        ...
        <View style={styles.gridCardBody}>
          <StyledText
            style={[styles.gridName, { color: colors.PRIMARY_TEXT }]}
            numberOfLines={1}
          >
            {item.name}
          </StyledText>
          ...
          <StyledText
            style={[
              styles.gridDate,
              { color: colors.SECTION_TEXT, fontSize: 10, marginTop: 2 },
            ]}
          >
          ...
          <View style={styles.gridMetadata}>
            {isToday ? (
              ...
              >
                ...
                >
                  {item.greetingSent
                    ? t("birthday_greeting_sent")
                    : t("birthday_greeting_not_sent")}
                </StyledText>
              </View>
            ) : (
              item.notificationId && (
                ...
                >
                  ...
                  <StyledText
                    style={{
                      fontSize: 9,
                      fontWeight: "600",
                      color: isReminderCancelled
                        ? colors.ERROR_INPUT_TEXT
                        : colors.PLACEHOLDER_SECOND,
                      textDecorationLine: isReminderCancelled
                        ? "line-through"
                        : "none",
                    }}
                  >
                    {notification?.date
                      ? formatDate(notification.date, lang)
                      : t("status_scheduled")}
                  </StyledText>
                </View>
              )
        ...
        >
          ...
          >
            {isToday ? (
              ...
              <StyledText
                style={[styles.avatarText, { color: colors.PRIMARY_TEXT }]}
              >
                {item.name.charAt(0).toUpperCase()}
              </StyledText>
            )}
          ...
          <View style={styles.cardContent}>
            ...
            <View style={styles.infoRow}>
              <StyledText
                style={[styles.dateText, { color: colors.SECTION_TEXT }]}
              >
                {formatBirthdayDate(item.date)}
              </StyledText>
            ...
            >
              ...
              >
                <StyledText
                  style={[styles.daysText, { color: getDaysColor(daysUntil) }]}
                >
                  {getDaysLabel(daysUntil)}
                </StyledText>
              </View>
            ...
            <View
              style={[styles.notificationStatus, { marginTop: 4, gap: 10 }]}
            >
              {isToday ? (
                <View
                  style={{ flexDirection: "row", alignItems: "center", gap: 4 }}
    ...
    return (
      <View style={{ flex: 1 }}>
        <View style={styles.header}>
          ...
          >
            <StyledText
              style={[styles.headerTitle, { color: colors.PRIMARY_TEXT }]}
            >
              {t("tab_birthday")}
            </StyledText>
          </View>
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      {(todayExpanded || upcomingExpanded || allExpanded) && (
        <TouchableOpacity
          style={[
          ...
          onPress={() =>
            toggleSection(setTodayExpanded, todayAnimation, "today")
          }
        ...
        >
          <View
            style={[
              ...
              todayBirthdays.length === 0 && { opacity: 0.5 },
            ]}
          >
            <View
              style={[
            ...
            <View style={styles.cardBadge}>
              <StyledText style={styles.cardBadgeText}>
          ...
          <View style={styles.sectionControls}>
            ...
            <View style={styles.chevronZone}>
              <Animated.View
                style={{
                  transform: [{ rotate: getRotation(todayAnimation) }],
                }}
              >
                <Ionicons
                  ...
                  style={[todayBirthdays.length === 0 && { opacity: 0.5 }]}
                />
              </Animated.View>
            </View>
          </View>
          <Animated.View
            style={[
              ...
              getCircleTransform(todayAnimation),
            ]}
          />
        </TouchableOpacity>
      )}

      {(upcomingExpanded || allExpanded) && (
        <TouchableOpacity
          style={[
            styles.sectionHeaderCard,
            styles.stickySectionHeader,
          ...
          onPress={() =>
            toggleSection(setUpcomingExpanded, upcomingAnimation, "upcoming")
          }
        ...
        >
          <View
            style={[
              styles.sectionTitleContainer,
              upcomingBirthdays.length === 0 && { opacity: 0.5 },
            ]}
          >
            <View
              style={[
          ...
          <View style={styles.sectionControls}>
            ...
            <View style={styles.chevronZone}>
              <Animated.View
                style={{
                  transform: [{ rotate: getRotation(upcomingAnimation) }],
                }}
              >
                <Ionicons
                  name="chevron-forward"
                  size={14}
                  color={isDark ? colors.SECTION_TEXT : colors.PRIMARY_TEXT}
                  style={[upcomingBirthdays.length === 0 && { opacity: 0.5 }]}
                />
              </Animated.View>
            </View>
          </View>
          <Animated.View
            style={[
              ...
              getCircleTransform(upcomingAnimation),
            ]}
          />
        </TouchableOpacity>
      )}
      ...
      {allExpanded && (
        <TouchableOpacity
          ...
          disabled={otherBirthdays.length === 0}
        >
          <View
            style={[
              styles.sectionTitleContainer,
              otherBirthdays.length === 0 && { opacity: 0.5 },
            ]}
          >
            <View
              style={[
          ...
          <View style={styles.sectionControls}>
            ...
            <View style={styles.chevronZone}>
              <Animated.View
                style={{ transform: [{ rotate: getRotation(allAnimation) }] }}
              >
                <Ionicons
                  name="chevron-forward"
                  size={14}
                  color={isDark ? colors.SECTION_TEXT : colors.PRIMARY_TEXT}
                  style={[otherBirthdays.length === 0 && { opacity: 0.5 }]}
                />
              </Animated.View>
            </View>
          </View>
          <Animated.View
            style={[
              ...
              getCircleTransform(allAnimation),
            ]}
          />
        </TouchableOpacity>
      )}
        ...
        {!upcomingExpanded && !allExpanded && (
          <View style={styles.sectionContainer}>
                  ...
                  <View style={styles.chevronZone}>
                    ...
                    >
                      <Ionicons
                        ...
                        size={14}
                        color={
                          isDark ? colors.SECTION_TEXT : colors.PRIMARY_TEXT
                        }
                        style={[
                          todayBirthdays.length === 0 && { opacity: 0.5 },
                        ]}
            ...
            {todayExpanded && (
              <View
                style={[
                  viewMode === "card" ? styles.gridContainer : { marginTop: 5 },
                ]}
              >
            ...
            >
              ...
              <View style={styles.sectionControls}>
                ...
                <View style={styles.chevronZone}>
                  <Animated.View
                    style={{
                      transform: [{ rotate: getRotation(allAnimation) }],
                    }}
                  >
                    <Ionicons
                      name="chevron-forward"
      ...
      <BirthdayMenuModal
        ...
        alreadyGreeted={
          !!(
            menuTarget?.greetingSent &&
            menuTarget?.greetingYear === new Date().getFullYear()
          )
        }
        onViewDetails={() => {
          if (menuTarget) {
          ...
          menuTarget?.notificationId
            ? (() => {
                const status = notifications.find(
                  (n) => n.id === menuTarget.notificationId,
                )?.status;
                return (
                  status === "Ləğv olunub" ||
