...
const TodoMenuModal: React.FC<TodoMenuModalProps> = ({
  ...
  const getMenuPosition = () => {
    ...
    if (!isCompleted && !isArchived) {
      ...
      if (onRetry && !isIterative) visibleItems.push("retry");
    ...
    const spaceBelow =
      windowHeight - (anchorPosition.y + anchorPosition.height + MARGIN);
    const shouldOpenUpwards =
      spaceBelow < ESTIMATED_HEIGHT && anchorPosition.y > ESTIMATED_HEIGHT;
    ...
    const isLeft =
      anchorPosition.x + anchorPosition.width / 2 < windowWidth / 2;
  ...
  const getVisibleItems = () => {
    ...
    if (!isCompleted && !isArchived) {
      ...
      if (archiveTodoAvailable && onArchive)
        items.push({
          id: "archive",
